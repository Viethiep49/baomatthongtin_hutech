...
private String preprocessText(String text, boolean encode) {
    ...
    private String processText(String text, boolean encode) {
        text = preprocessText(text, encode);
        ...
        for (int i = 0; i < text.length(); i += 2) {
            ...
            if (posA[0] == posB[0]) {
                result.append(keyMatrix[posA[0]][(posA[1] + (encode ? 1 : 4)) % MATRIX_SIZE]);
                result.append(keyMatrix[posB[0]][(posB[1] + (encode ? 1 : 4)) % MATRIX_SIZE]);
            } else if (posA[1] == posB[1]) {
                result.append(keyMatrix[(posA[0] + (encode ? 1 : 4)) % MATRIX_SIZE][posA[1]]);
                result.append(keyMatrix[(posB[0] + (encode ? 1 : 4)) % MATRIX_SIZE][posB[1]]);
            } else {
